                        ...
                        <div class="col-12 col-md-6 mx-auto">
                            <div class="card card-default shadow">
                                <div class="card-header">
                                    <h4 class="card-title">A2C(Account to Customer) balance transfer</h4>
                                </div><!--end card-header-->
                                <div class="card-body">
                                    @if(count($customer)>0)
                                    <form class="form" method="Post" action="{{ route('confirmA2CTransfer') }}">
                                        {{ csrf_field() }}
                                        <input type="hidden" name="profileid" value="{{ $customer->id }}">
                                        <div class="form-group pb-2">
                                            <label for="customerId">User ID</label>
                                            <input type="text" name="customerId" class="form-control" placeholder="Enter transfer user id" required>
                                        </div>
                                        <div class="form-group pb-2">
